...
-- Save to JSON file
local function SaveDepositedPets()
    local jsonData = HttpService:JSONEncode(DepositedPets)
    writefile("DepositedPets.json", jsonData)
    print("💾 DepositedPets saved to DepositedPets.json")
end

-- Load from JSON file
local function LoadDepositedPets()
    if isfile("DepositedPets.json") then
        local jsonData = readfile("DepositedPets.json")
        local loadedData = HttpService:JSONDecode(jsonData)
        DepositedPets = loadedData or {}
        print("📂 DepositedPets loaded from DepositedPets.json")
    else
        DepositedPets = {}
...
-- Build dictionary of owned pets by name
...
-- Deposit all pets of a given name
...
-- Withdraw all pets of a given name
