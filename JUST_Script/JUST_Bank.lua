...
-- ======================================================
-- FUNCTIONS (Unified with IF/ELSE logic)
-- ======================================================

-- ✅ Deposit (Pets+Gems / Pets only / Gems only)
local function Deposit(bankId, petName, diamondAmount)
    ...
    if #uids > 0 and diamondAmount > 0 then
        print("✅ Depositing Pets + Gems (max 600)")
        BankDeposit:InvokeServer(bankId, {unpack(uids, 1, 600)}, diamondAmount)

    elseif #uids > 0 then
        print("✅ Depositing Pets Only (max 600)")
        BankDeposit:InvokeServer(bankId, {unpack(uids, 1, 600)}, 0)

    elseif diamondAmount > 0 then
        print("✅ Depositing Gems Only")
...
-- ✅ Withdraw (Pets+Gems / Pets only / Gems only)
local function Withdraw(bankId, petName, diamondAmount)
    local uids = {}

    if petName and petName ~= "" then
        -- find petId from Library
        local petId
        for id, data in pairs(Library.Directory.Pets) do
            if data.name:lower() == petName:lower() then
                petId = id
                break
            end
        end
        ...
        if petId then
            ...
            if ok and bankData and bankData.Storage and bankData.Storage.Pets then
    ...
    -- decide mode
    if #uids > 0 and diamondAmount > 0 then
        print("✅ Withdrawing Pets + Gems (max 600)")
        BankWithdraw:InvokeServer(bankId, {unpack(uids, 1, 600)}, diamondAmount)

    elseif #uids > 0 then
        print("✅ Withdrawing Pets Only (max 600)")
        BankWithdraw:InvokeServer(bankId, {unpack(uids, 1, 600)}, 0)

    elseif diamondAmount > 0 then
        print("✅ Withdrawing Gems Only")
...
CreateButton("Deposit", 70, function()
    if SelectedBankID then
        local diamonds = tonumber(DiamondBox.Text) or 0 -- get user input, default 0
...
print("✅ BoogyMan Bank UI Loaded with Unified Withdraw & Deposit (Pets+Gems / Pets Only / Gems Only)")
