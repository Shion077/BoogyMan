...
frame.Size = UDim2.new(0, 150, 0, 170) -- Increased height to fit toggle button
...
-- Get player's Humanoid (wait for character if needed)
...
local function startAutoJump()
    if jumpingConnection then return end -- Already jumping
    jumpingConnection = RunService.Heartbeat:Connect(function()
...
-- Toggle via touch on statusCircle only
...
selectPositionButton.TouchTap:Connect(function()
	...
	conn = UserInputService.InputBegan:Connect(function(input)
		if input.UserInputType == Enum.UserInputType.Touch then
			ClickPosition = Vector2.new(round(input.Position.X,1), round(input.Position.Y,1))
